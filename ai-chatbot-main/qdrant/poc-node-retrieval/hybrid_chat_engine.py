...
def relative_score_fusion(
    ...
    sparse_per_node = {
        sparse_result_tuples[i][1].node_id: x for i, x in enumerate(sparse_similarities)
    }
    ...
    dense_per_node = {
        dense_result_tuples[i][1].node_id: x for i, x in enumerate(dense_similarities)
    }
...
aclient = AsyncQdrantClient(
    url="http://localhost:6334", api_key=api_key, prefer_grpc=True
)
...
chat_engine = index.as_chat_engine(
    ...
    sparse_top_k=12,
    alpha=0.2,
    vector_store_query_mode="hybrid" # ลืมใส่ตรงนี้
)

response = chat_engine.chat("What is lotuss coin?")
