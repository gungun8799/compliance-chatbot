...
async def answer_with_llm(nodes: list, query: str, level: str, top_score: float, fuzzy_score: float):
    ...
    if suggest_table:
        formatting_hint = (
            "\n🧮 คำแนะนำสำคัญ: คำถามของผู้ใช้มีการระบุ 'มูลค่า' ที่ชัดเจน กรุณาเปรียบเทียบมูลค่านี้กับช่วงที่ระบุในเอกสาร "
            "และเลือกช่วงที่ครอบคลุมมูลค่าดังกล่าวให้ถูกต้องที่สุด คิดทบทวนให่้ดีอย่าตอบมั่วหรือสร้างคำตอบขึ้นเอง ถ้าไม่แน่ใจให่้ถามกลับ "
            "\n\nหากสามารถจัดให้อยู่ในรูปแบบ **ตาราง Markdown** ได้ กรุณาระบุหัวข้อหลัก เช่น 'ประเภทโครงการ', 'มูลค่า', 'ผู้มีอำนาจอนุมัติ' เป็นต้น"
            "\nหากไม่สามารถจัดตารางได้ ให้สรุปแบบ bullet โดยอ้างอิงตามข้อความต้นฉบับ"
        )
    else:
    ...
    filtered_query = (
        ...
        f"{formatting_hint}\nในคำตอบให้ระบุชื่อเอกสารนโยบายที่ใช้ คิดทบทวนให่้ดีอย่าตอบมั่วหรือสร้างคำตอบขึ้นเอง ถ้าไม่แน่ใจให่้ถามกลับ และชี้ว่าอ้างอิงมาจากข้อความใดบ้าง"
    )
