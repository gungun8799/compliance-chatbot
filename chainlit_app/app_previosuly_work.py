...
# Chat Store - Test Redis connectivity
try:
    ...
    if test_result:
        ...
        logger.warning("⚠️ Redis test failed - memory may not persist")
except Exception as e:
    logger.error(f"❌ Redis connection failed: {e}")
    logger.info("🔄 Chat memory will use fallback mode")
    chat_store = None
    ...
    if memory is None:
        ...
        try:
            if chat_store is not None:
                ...
                logger.info("🔄 Memory created with in-memory fallback (Redis unavailable)")
            cl.user_session.set("memory", memory)
